...
const FilterMenu = ({isModalOpen, setModalOpen}: FilterMenuProps) => {
    ...
    const FilterForm = () => {
        return (
            <div
                className="sm:w-5/5 md:w-2/10 mb-4 h-[739px] rounded-3xl border bg-zinc-100 p-9 backdrop-blur-3xl sm:!m-3 md:mb-0 min-w-[100px]">
                <div className={"absolute block md:hidden"} style={{right: "15px", top: "15px"}}>
                    <button
                        onClick={closeModal}
                        className="z-20 text-black font-bold rounded"
                    >
    ...
    return (
        <div className={"inset-0 h-[auto] w-[auto]"}>
            {isModalOpen ? (
                <div className="inset-0 absolute h-[150%] md:h-[auto] z-10 bg-black bg-opacity-50 flex items-center justify-center">
                    {FilterForm()}</div>) : (<div className={"hidden md:block"}>{FilterForm()}</div>)}
        </div>
    )
}
